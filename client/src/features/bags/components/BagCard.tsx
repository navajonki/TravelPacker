...
import { BagWithItems, BaseCardProps } from '@shared/types';
import { ItemApi } from '@/api/apiClient';
import { invalidateBags, invalidateCategories, invalidateTravelers } from '@/services/queryInvalidation';
import { createLogger } from '@/services/logging';
import ItemRow from "@/components/ItemRow"; // Will be migrated to features/items/components later

// Create a bag-specific logger
const logger = createLogger('bags');

interface BagCardProps extends BaseCardProps {
  bag: BagWithItems;
...
export default function BagCard({ 
  ...
  const bulkActionMutation = useMutation({
    mutationFn: async (action: "pack" | "unpack" | "packRemaining") => {
      logger.debug('Performing bulk action on bag items', { 
        bagId: bag.id, 
        action, 
      ...
      if (action === "packRemaining") {
        ...
        return await fetch(`/api/bags/${bag.id}/bulk-update-items`, {
          method: 'PATCH',
